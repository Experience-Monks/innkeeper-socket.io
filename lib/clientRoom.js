...
function room( ioClient, roomID, roomData, roomUsers ) {
  ...
  ioClient.on( events.ROOM_VARIABLE, function( data ) {
    ...
    if( data.roomID == roomID ) {
      ...
      this.emit( 'data', this.roomData, data );
    }
  }.bind( this ));
  ...
  ioClient.on( events.ROOM_DATA, function( data ) {
    ...
    if( data.roomID == roomID ) {
      ...
      this.roomData = data;
      this.emit( 'data', this.roomData, null ); 
    }
  }.bind( this ));
}
...
/**
 * Call init to ensure that all room data and users are setup
 * 
 * @return {Promise} This promise will resolve once the client is fully initialized
 */

p.init = function() {

  this.ioClient.emit( events.ROOM_INIT, function( data ) {
...
p.setVar = function( key, value ) {
  ...
  return new promise( function( ok, err ) {
    ...
    ioClient.emit( events.ROOM_VARIABLE, {
...
p.setRoomData = function( data ) {
  ...
  return new promise( function( ok, err ) {
    ...
    ioClient.emit( events.ROOM_DATA, {
